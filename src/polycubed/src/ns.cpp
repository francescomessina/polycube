...
std::thread Namespace::executor(Namespace::worker);
std::condition_variable Namespace::cv;
std::mutex Namespace::mutex;
std::queue<Event *> Namespace::queue;
...
Namespace Namespace::create(const std::string &name) {
  ...
  auto f = [&]() {
    ...
    ::snprintf(netns_path, sizeof(netns_path), "%s/%s", NETNS_RUN_DIR, name.c_str());
...
Namespace Namespace::open(const std::string &name) {
  ...
  auto f = [&]() {
    ...
    fd = ::open(netns_path, O_RDONLY|O_EXCL, 0);
    if (fd < 0) {
      throw std::runtime_error(std::string("Error opening namespace: ") + std::strerror(errno));
    }
  };
...
void Namespace::execute(std::function<void()> f) {
  std::function<void()> f_ = [&]() {
    if (setns(fd_, CLONE_NEWNET)) {
      throw std::runtime_error(std::string("setns error: ") + std::strerror(errno));
    }
    f();
  };
...
void Namespace::remove() {
  char netns_path[PATH_MAX];
  snprintf(netns_path, sizeof(netns_path), "%s/%s", NETNS_RUN_DIR, name_.c_str());
  ::close(fd_);

  ::umount2(netns_path, MNT_DETACH);
  if (unlink(netns_path) < 0) {
    throw std::runtime_error(std::string("umount error : ") + std::strerror(errno));
  }
}
...
void Namespace::create_ns(const std::string &name) {
  ...
  if (fd < 0) {
    throw std::runtime_error(std::string("open error: ") + std::strerror(errno));
  }

  ::close(fd);

  if(::unshare(CLONE_NEWNET) == -1) {
    throw std::runtime_error(std::string("unshare error: ") + std::strerror(errno));
  }
  ...
  if(::mount(os.str().c_str(), netns_path.c_str(), "none", MS_BIND, NULL)) {
    throw std::runtime_error(std::string("mount error: ") + std::strerror(errno));
  }
}
