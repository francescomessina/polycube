...
void SimplebridgeApi::setup_routes() {
  ...
  Routes::Get(router, base + ":name/shadow/", Routes::bind(&SimplebridgeApi::read_simplebridge_shadow_by_id_handler, this));
  ...
  Routes::Patch(router, base + ":name/shadow/", Routes::bind(&SimplebridgeApi::update_simplebridge_shadow_by_id_handler, this));
...
void SimplebridgeApi::read_simplebridge_shadow_by_id_handler(
  const polycube::service::Rest::Request &request,
  polycube::service::HttpHandleResponse &response) {
  // Getting the path params
  auto name = request.param(":name").as<std::string>();
  ...
  try {
    ...
    auto x = read_simplebridge_shadow_by_id(name);
    nlohmann::json response_body;
    response_body = x;
    response.send(polycube::service::Http::Code::Ok, response_body.dump(4));
...
void SimplebridgeApi::update_simplebridge_shadow_by_id_handler(
  const polycube::service::Rest::Request &request,
  polycube::service::HttpHandleResponse &response) {
  // Getting the path params
  auto name = request.param(":name").as<std::string>();
  ...
  try {
    ...
    // The conversion is done automatically by the json library
    value = request_body;
    update_simplebridge_shadow_by_id(name, value);
    response.send(polycube::service::Http::Code::Ok);
  } catch(const std::exception &e) {
    response.send(polycube::service::Http::Code::Internal_Server_Error, e.what());
  }
