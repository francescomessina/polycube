...
// BPF table of single element that saves the span mode attribute
BPF_TABLE("hash", int, bool, span_, 1);
...
static inline int send_packet_to_output_interface(
  ...
  /* Span mode check */
  unsigned int zero = 0;
  bool *span = span_.lookup(&zero);
  if (!span) {
    return RX_DROP;
  }
  if (*span) {
    // send packet to slowpath to be duplicated
    u32 mdata[3];
    mdata[0] = out_port;
    return pcn_pkt_controller_with_metadata(ctx, md, SLOWPATH_SPAN_MODE, mdata);
  }
